...
const AddProduct = () => {
    ...
    const [input, setInput] = useState({
        productName: "", productDesc: "", productPrice: "", category: "", brand: ""
    });
    ...
    //         formData.append("files", files[i]); // "files" naam backend controller se match hona chahiye
    //     }
    ...
    //             navigate("/admin/products"); // Success ke baad list par wapas
    //         }
    //     } catch (error) {
...
const submitHandler = async (e) => {
    ...
    try {
        // Sahi URL path: /api/user/product/add
        const res = await axios.post("http://localhost:5000/api/user/product/add", formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
            withCredentials: true
        });
        if (res.data.success) {
                ...
                <form onSubmit={submitHandler} className='space-y-5'>
                    <div className='grid grid-cols-1 gap-4'>
                        ...
                        <div className='grid grid-cols-2 gap-4'>
                            <input type="number" name="productPrice" value={input.productPrice} onChange={changeEventHandler} placeholder="Price (₹)" className='border p-3 rounded-xl outline-none'/>
                        ...
                        <div className='border-2 border-dashed border-gray-200 p-6 rounded-xl text-center bg-gray-50'>
                            <input type="file" multiple onChange={(e) => setFiles(e.target.files)} id="file-upload" className='hidden'/>
