...
const Payment = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 2. Razorpay Options (Updated with new Key)
      const options = {
        // Yahan maine updated key dali hai jo aapne .env mein dikhayi thi
        key: import.meta.env.VITE_RAZORPAY_KEY_ID || "rzp_test_SjzPCZYvQKABgm", 
        amount: order.amount,
        currency: "INR",
        name: "Upendra's Store",
        description: "Secure Payment for your Order",
        image: "", // Aap apna logo URL yahan daal sakte hain
        order_id: order.id,
        handler: async (response) => {
          try {
            const verifyData = {
