...
const Payment = () => {
  ...
  // Total Calculation: Subtotal + 18% GST
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        image: "", // Fixed: Empty string to avoid localhost CORS error
        order_id: order.id,
        handler: async (response) => {
          try {
            // Hum backend ko verification details + order ka data bhej rahe hain
            const verifyData = {
            ...
            const res = await axios.post('http://localhost:5000/api/v1/payment/verify', verifyData, config);
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-gray-50 max-w-md mx-auto text-center">
        ...
        <div className="border-2 border-blue-50 bg-blue-50/30 p-6 rounded-3xl mb-8">
          ...
          <div className="flex justify-between items-center mb-4 pb-4 border-b border-blue-100">
            ...
            <span className="text-gray-800 font-bold">₹ {Math.round(subtotal * 0.18).toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600 font-black">Total Payable:</span>
            <span className="text-3xl font-black text-blue-600 tracking-tighter">₹ {totalAmount.toLocaleString()}</span>
          </div>
        </div>
